...
namespace YTEngine {
	...
	public:
		...
		*/
		...
		{
			static ShadowMap* instance = nullptr;
			if (instance == nullptr) {
				instance = new ShadowMap;//シャドウマップのインスタンスを作成。
			}
			return *instance;
		}
	private:
	...
	/*!
	* @brief	ShadowMapのインスタンスを取得。
	*/
	static inline ShadowMap& Shadow_map()
	{
		return ShadowMap::GetInstance();
	}
}
