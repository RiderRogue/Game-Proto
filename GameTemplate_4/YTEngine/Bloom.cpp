...
namespace YTEngine {
	...
	{
		g_graphicsEngine->BeginGPUEvent(L"Bloom::Begin");
		...
		//まずは輝度を抽出する。
		{
			...
			//シーンをテクスチャとする。
			auto mainRTTexSRV = GameObjectManager().GetMainRenderTarget()->GetRenderTargetSRV();
		...
		{
			auto mainRT = GameObjectManager().GetMainRenderTarget();
